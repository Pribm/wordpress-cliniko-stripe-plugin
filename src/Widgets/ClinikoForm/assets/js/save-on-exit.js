(function() {
  ...
  function getSteps(root){ return $$(".form-step", root); }
  function getActiveStepIndex(root){
    const steps = getSteps(root);
    if (!steps.length) return 0;
    let idx = steps.findIndex(s => s.style.display !== "none" && !s.classList.contains("is-hidden"));
    if (idx < 0) idx = steps.findIndex(s => s.style.display !== "none");
    if (idx < 0) idx = steps.findIndex(s => !s.classList.contains("is-hidden"));
    if (idx < 0) idx = 0;
  ...
  function syncNavButtons(root){
    ...
    const evt = new CustomEvent("stepchange", { detail: { index: idx, total } });
    document.dispatchEvent(evt);
  }

  // ===== Save / Load =====
  function serializeForm(root){
    const stepIndex = (typeof window.currentStep === "number")
      ? window.currentStep
    ...
    if (canvas && canvas.toDataURL) { try{ data.signatureDataUrl = canvas.toDataURL("image/png"); }catch(_){} }
  ...
  function applyValues(root,data){
    ...
    steps.forEach((s,i)=>{
      ...
      s.classList.toggle("is-hidden", !active);
    });
    ...
    if(data.signatureDataUrl){
      const canvas = $("#signature-pad", root), hidden=$("#signature-data", root);
      if(canvas){
  ...
  function saveAll(root){ try{ localStorage.setItem(STORAGE_KEY, JSON.stringify(serializeForm(root))); return true; }catch(e){ return false; } }
  function loadSaved(){ try{ const r=localStorage.getItem(STORAGE_KEY); return r?JSON.parse(r):null; }catch(_){ return null; } }
  ...
  function showExitModal(){
    ...
    return new Promise(resolve=>{
      ...
      const finish=(a)=>{ backdrop.remove(); resolve(a); };
      backdrop.addEventListener('click',e=>{ if(e.target===backdrop) finish('cancel'); });
      backdrop.querySelectorAll('button').forEach(btn=>btn.addEventListener('click',()=>finish(btn.getAttribute('data-action'))));
    });
  }
  ...
  function showRestoreBar(saved,root){
    ...
    bar.innerHTML=`
      <span>We found your appointment form from 
        <span class="timestamp">${new Date(saved.when).toLocaleString()}</span>. 
    ...
    bar.addEventListener('click',(e)=>{
      ...
      if(action==="resume"){ 
        ...
        showToast("Your appointment form has been restored. You can continue where you left off.","success"); 
        bar.remove(); 
      }
      if(action==="discard"){ 
        clearSaved(); 
  ...
  function observeStepChanges(root){
    ...
    const update = debounce(()=>{
      let raw = loadSaved();
      if(!raw || typeof raw!=="object") raw = { values:{}, when:Date.now(), stepIndex:0 };

      raw.stepIndex = (typeof window.currentStep === "number")
        ? window.currentStep
        : getActiveStepIndex(root);

      raw.when = Date.now();
      try{ localStorage.setItem(STORAGE_KEY, JSON.stringify(raw)); }catch(_){}
      progressTo(raw.stepIndex,total);
      syncNavButtons(root);
    }, 60);

    const mo = new MutationObserver(()=>update());
    mo.observe(root, { subtree:true, attributes:true, attributeFilter:["style","class"] });

    const prev=$(PREV_BTN_SEL), next=$(NEXT_BTN_SEL);
    [prev,next].forEach(btn=>{
      if(!btn) return;
      btn.addEventListener("click", ()=> setTimeout(()=>{ update(); }, 0), true);
    });
  ...
  ready(()=>{
    ...
    if(!root){ console.warn("[ExitSave] Form root not found:", FORM_ROOT_SEL); return; }
    ensureStyles();

    const saved = loadSaved();
    if (saved){ 
      showRestoreBar(saved, root); 
      showToast("We found a saved appointment form. You can continue it from the top of the page.","success"); 
    }
    else { 
      showToast("Don’t worry, your appointment details will be saved automatically if you leave.","success"); 
    }
    ...
    $$("input, textarea, select", root).forEach(el=>["input","change","blur"].forEach(evt=>el.addEventListener(evt, debouncedSave, true)));
    ...
    document.addEventListener('click', async (e)=>{
      ...
      if(!href || href.startsWith('#') || a.target==='_blank' || href.startsWith('mailto:') || href.startsWith('tel:') || href.startsWith('javascript:')) return;
      e.preventDefault(); pendingNav={type:'link',href};
      const action=await showExitModal();
      if(action==='cancel'){ pendingNav=null; return; }
      if(action==='save-leave'){ saveAll(root); showToast("Your appointment details have been saved. Redirecting…","success"); }
    ...
    document.addEventListener('submit',(e)=>{ 
      if(e.target && root.contains(e.target)) {
        ...
        saveAll(root); 
        showToast("We’ve saved your appointment details while confirming your booking.","success"); 
