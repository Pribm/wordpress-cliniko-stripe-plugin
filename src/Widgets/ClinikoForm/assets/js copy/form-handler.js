...
function mountForm() {
  ...
  function isCurrentStepValid() {
    ...
    for (let field of currentFields) {
      ...
      if (
        ...
        const passField = steps[window.currentStep].querySelector(
          'input[name="patient[password]"]'
        );
        const confirmField = steps[window.currentStep].querySelector(
          'input[name="patient[password_confirmation]"]'
        );
        ...
        if (passField && confirmField) {
          ...
          if (!passVal || !confirmVal) {
            [passField, confirmField].forEach(
              (f) => (f.style.borderColor = "red")
            );
            isValid = false;
            const msg = document.createElement("div");
            msg.className = "field-error";
  ...
  nextBtn.addEventListener("click", async () => {
    ...
    if (window.currentStep < steps.length - 1) {
      ...
      // Special handling: account choice step
      const currentStepEl = steps[window.currentStep];
      const choiceInput = currentStepEl.querySelector(
        "[name=has_account]:checked"
      );
      if (choiceInput) {
        if (choiceInput.value === "yes") {
          goToStep(window.currentStep + 1); // show login
          return;
        } else {
          goToStep(window.currentStep + 2); // skip login, go to register
          return;
        }
      }
  ...
  prevBtn.addEventListener("click", () => {
    if (window.currentStep > 0) {
      // If we're in login or register, go back to account choice
      const currentStepEl = steps[window.currentStep];
      if (
        currentStepEl.querySelector("#login-email") ||
        currentStepEl.querySelector("#patient-password")
      ) {
        goToStep(window.currentStep - 2);
      } else {
        goToStep(window.currentStep - 1);
      }
    }
